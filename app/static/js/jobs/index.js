...
function renderJobPanelItem(job) {
    ...
    <li class="jobs-panel-item" data-job-id="${job.id}">
      ...
      <div class="jobs-panel-item-actions">
        ${active
        ? `<button type="button" class="danger-btn jobs-panel-stop" data-stop-job="${job.id}" ${stopping ? "disabled" : ""}>${escapeHtml(stopping ? t("jobs.status.cancelled") : t("jobs.stop"))}</button>`
        : ""}
...
export function initJobsPanelHandlers() {
    jobsPanelListEl?.addEventListener("click", (event) => {
        ...
        if (stopBtn) {
            ...
            if (jobId)
                cancelBackgroundJob(jobId).catch(() => { });
            return;
        }
...
export function renderBackgroundJobsBar() {
    ...
    ${active
        .map((job) => `<span class="background-job-chip">${escapeHtml(backgroundJobLabel(job))}</span>`)
        .join("")}
...
function applyEnrichContactJobResult(job) {
    ...
    notifySuccess(t("jobs.enrichDone", {
        count,
        contactId,
...
export function finishBackgroundJob(job) {
    ...
    if (job.status === "done") {
        if (job.job_type === "lookup")
            applyLookupJobResult(job);
        else if (job.job_type === "lead_discover")
            applyLeadDiscoverJobResult(job);
        else if (job.job_type === "enrich_contact")
            applyEnrichContactJobResult(job);
...
function startPollingForJob(jobId) {
    const entry = backgroundJobTrackers.get(jobId);
    if (!entry || entry.timer || jobsSseConnected)
        return;
