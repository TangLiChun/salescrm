...
function focusables(modalEl) {
    return [...dialogRoot(modalEl).querySelectorAll(FOCUSABLE)].filter((el) => !el.closest(".hidden") && el.offsetParent !== null);
}
...
export function openModal(modalEl, { initialFocus } = {}) {
    ...
    stack.push(entry);
    const target = (initialFocus && dialogRoot(modalEl).querySelector(initialFocus)) ||
...
export function handleModalKeydown(event) {
    ...
    if (!entry || !isModalOpen(entry.modalEl))
        return;
    if (event.key === "Escape") {
