...
function renderPrefTags(items, tone = "neutral") {
    ...
    return items
        .map((item) => `<span class="lead-prefs-tag lead-prefs-tag--${tone}">${escapeHtml(String(item))}</span>`)
        .join("");
}
...
export async function loadLeadPreferences() {
    ...
    try {
        ...
        const statItems = LEAD_PREF_STAT_META.map(({ key, field, tone }) => `
...
function renderAiSetupChecklist(data) {
    ...
    const item = (done, doneKey, pendingKey) => `
...
export async function loadSettingsForm() {
    ...
    setInputValue("setting-brightdata-web-unlocker-max-urls", data.brightdata_web_unlocker_max_urls || "6");
...
export async function saveSettings(event) {
    ...
    const payload = {
        ...
        brightdata_web_unlocker_enabled: document.getElementById("setting-brightdata-web-unlocker-enabled").checked
            ? "1"
            : "0",
        brightdata_web_unlocker_max_urls: String(Math.max(1, Math.min(12, Number(document.getElementById("setting-brightdata-web-unlocker-max-urls").value) || 6))),
        ...
        email_send_interval_minutes: document.getElementById("email-send-interval-minutes").value.trim() || "5",
        email_daily_cap: document.getElementById("email-daily-cap").value.trim() || "50",
        email_active_start_hour: document.getElementById("email-active-start-hour").value.trim() || "9",
