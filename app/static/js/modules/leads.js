...
import { api, escapeHtml, formatImportResult, normalizeImportRows } from "../core/utils.js";
...
const { leadQueryInput, minScoreInput, autoImportInput, discoverBtn, discoverViaPiBtn, aiProgressEl, aiProgressFill, aiProgressText, aiStatsEl, aiLeadsBody, aiLeadsStateEl, aiChannelsEl, importLeadsBtn, retryDiscoverBtn, leadDetailModal, leadDetailBody, leadDetailImport, discoverBackgroundInput, } = dom;
...
export function setChannel(key, patch) {
    if (!state.channelState[key])
        state.channelState[key] = { state: "idle", count: "", preview: "" };
...
export async function loadLlmStatus() {
    try {
        ...
        if (state.llmConfigured) {
            ...
            if (discoverViaPiBtn)
                discoverViaPiBtn.disabled = false;
            hideLeadsState();
        }
        else {
            discoverBtn.disabled = true;
            if (discoverViaPiBtn)
                discoverViaPiBtn.disabled = true;
            hideLeadsState();
        }
    }
    catch {
...
export function setDiscoverRunning(running) {
    if (running) {
        ...
        if (discoverViaPiBtn)
            discoverViaPiBtn.disabled = !state.llmConfigured;
    }
}
...
export async function importAiLeads() {
    ...
    const rows = normalizeImportRows(selected.map((lead) => ({
        ...lead,
        source: "ai-lead",
