...
import { CHANNEL_DEFS, PI_LEAD_STREAM_TOOLS, PI_CHANNEL_ICON, PI_SOURCE_CHANNEL_MAP, PI_CHAT_STORAGE_VERSION, PI_THREADS_STORAGE_VERSION, PI_CHAT_MAX_STORED, PI_THREADS_MAX, } from "../core/state.js";
...
const { piChatMessagesEl, piChatForm, piChatInput, piChatSendBtn, piChatStopBtn, piChatClearBtn, piChatHistoryHintEl, piChatContextMeterEl, piChatContextMeterFillEl, piChatContextMeterLabelEl, piChatBackgroundInput, piThreadListEl, piChatProgressEl, piChatProgressFill, piChatProgressText, piChatJumpBtn, piChatStatusLiveEl, leadQueryInput, minScoreInput, autoImportInput, piChatShellEl, piLlmSetupEl, piGotoLlmSettingsBtn, piInputHintEl, piMobileTabChatBtn, piMobileTabThreadsBtn, } = dom;
...
export function renderPiEmptyStateHtml() {
    ...
    const chips = PI_EXAMPLE_PROMPTS.map(({ labelKey, messageKey }) => `<button type="button" class="pi-example-chip" data-pi-example="${escapeHtml(t(messageKey))}">${escapeHtml(t(labelKey))}</button>`).join("");
    return `
    <div class="pi-chat-empty pi-chat-empty-state">
...
export function refreshPiToolLabelsInDom() {
    piChatMessagesEl?.querySelectorAll(".pi-chat-tool[data-tool-name]").forEach((el) => {
        ...
        if (labelEl && name)
            labelEl.textContent = formatPiToolLabel(name);
    });
}
...
export function initPiAgentUi() {
    ...
    piChatMessagesEl?.addEventListener("click", (event) => {
        ...
        if (!chip || !piChatInput || state.piChatBusy || !state.llmConfigured)
            return;
...
export function recoverLegacyPiHistory(userId) {
    ...
    if (state.piThreads.some((thread) => thread.history?.length && historyFingerprint(thread.history) === legacyFp)) {
        return false;
    }
    const emptyMatch = state.piThreads.find((thread) => {
        if (thread.history?.length)
            return false;
        if (!thread.title || !legacy.title)
            return false;
...
export function savePiThreadsStoreLocal() {
    ...
    syncActivePiThreadHistory();
    try {
        localStorage.setItem(piThreadsStorageKey(state.currentUserId), JSON.stringify({
...
export async function switchPiThread(threadId) {
    if (threadId === state.activePiThreadId)
        return;
    if (state.piChatBusy) {
        notifyInfo(t("pi.busySwitch"));
        return;
...
export async function deletePiThread(threadId) {
    ...
    if (!window.confirm(t("pi.confirmDeleteThread")))
        return;
    try {
...
export function updatePiContextMeter() {
    if (!piChatContextMeterEl || !piChatContextMeterFillEl || !piChatContextMeterLabelEl)
        return;
    ...
    if (stats.compressed)
        parts.push(t("msg.piContextCompressed"));
    else if (stats.needs_compression)
        parts.push(t("msg.piContextNeedsCompress"));
    if (pct >= 75)
        parts.push(t("msg.piContextHigh"));
    piChatContextMeterLabelEl.textContent = parts.join(" · ");
}
export function updatePiChatHistoryHint() {
    if (!piChatHistoryHintEl)
        return;
...
export function piSelectableRows(toolEl) {
    return Array.from(toolEl?.querySelectorAll(".pi-chat-leads-body tr, .pi-chat-lookup-body tr") || []);
}
...
export function updatePiSelectionUi(toolEl) {
    ...
    if (countEl)
        countEl.textContent = total ? t("pi.selectedCount", { n: selected, total }) : "";
...
export function wirePiSelection(toolEl) {
    ...
    if (body && !body.dataset.selBound) {
        ...
        body.addEventListener("change", (event) => {
            const cb = event.target?.closest(".pi-row-check");
            if (!cb)
                return;
...
export async function importPiChatLookup(toolEl) {
    ...
    const rows = piSelectableRows(toolEl)
        ...
        .map((tr) => allRows[Number(tr.dataset.index)]);
    const payload = normalizeImportRows(rows.filter((row) => row?.email).map((row) => ({
        ...row,
...
export function startPiDiscoverTimer(toolEl) {
    ...
    toolEl._piDiscoverTimer = setInterval(() => {
        ...
        if (!elapsedEl || !toolEl._piDiscoverStartedAt)
            return;
...
export function renderPiDiscoverLivePanel(toolEl) {
    ...
    if (!channelsEl || !toolEl._piChannelState)
        return;
...
export function handlePiDiscoverToolEvent(toolEl, event) {
    ...
    switch (event.kind) {
        ...
        case "progress":
            ...
            break;
        case "asn_result":
            pushPiDiscoverTicker(toolEl, t("pi.asnCandidates", {
                asn: event.asn,
...
export async function importPiChatLeads(toolEl) {
    ...
    const rows = piSelectableRows(toolEl)
        .filter((tr) => piRowCheckbox(tr)?.checked)
        .map((tr) => allLeads[Number(tr.dataset.index)]);
    const payload = normalizeImportRows(rows
...
export function updatePiChatAssistantBubble(el, text, streaming = false) {
    ...
    if (piChatStreamRaf)
        cancelAnimationFrame(piChatStreamRaf);
...
export function announcePi(message) {
    ...
    requestAnimationFrame(() => {
        if (piChatStatusLiveEl)
            piChatStatusLiveEl.textContent = text;
    });
}
...
export function applyPiDestructiveHint(toolEl, name) {
    if (!toolEl || !PI_DESTRUCTIVE_TOOLS.has(name))
        return;
    toolEl.classList.add("pi-chat-tool-destructive");
    if (toolEl.querySelector(".pi-chat-destructive-note"))
        return;
    const note = document.createElement("p");
...
export function showPiToolRawDetail(toolEl, text) {
    if (!toolEl || !text)
        return;
    let details = toolEl.querySelector(".pi-chat-tool-raw");
    if (!details) {
...
export function addPiUndoButton(toolEl, result) {
    if (!toolEl || !result?.undo_payload || toolEl.querySelector(".pi-undo-btn"))
        return;
...
async function finalizePiBackgroundJob(job) {
    ...
    if (!piBackgroundThreadMatches(job))
        return;
...
export async function retryLastPiTurn() {
    if (state.piChatBusy || !state.llmConfigured)
        return;
    let idx = -1;
        ...
        while (true) {
            ...
            for (const chunk of chunks) {
                ...
                if (payload.type === "context") {
                    ...
                    if (activeToolEl?.classList.contains("pi-chat-tool-discover") &&
                    ...
                    if (activeToolEl) {
                        ...
                        if (!toolFailed) {
                            ...
                            if (payload.result?.undo_payload)
                                addPiUndoButton(toolEl, payload.result);
                        }
                    }
