...
export function contactActionsHtml(contact) {
    ...
    const menuItem = (className, label, extra = "") => `<button type="button" class="action-menu-item link-btn ${className}" role="menuitem" data-id="${id}"${extra}>${label}</button>`;
    return `
...
export function updateContactsMetrics(total, sentOnPage, unsentOnPage) {
    if (contactsMetricTotalEl)
        contactsMetricTotalEl.textContent = String(total ?? 0);
    if (contactsMetricSentEl)
        contactsMetricSentEl.textContent = String(sentOnPage ?? 0);
    if (contactsMetricUnsentEl)
        contactsMetricUnsentEl.textContent = String(unsentOnPage ?? 0);
}
...
export async function deleteContact(contactId) {
    if (!confirm(t("msg.confirmDeleteContact")))
        return;
...
export async function deleteContactNote(noteId) {
    if (!state.notesContactId)
        return;
    if (!confirm(t("msg.confirmDeleteNote")))
        return;
