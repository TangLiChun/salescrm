...
function syncViewHash(view: string) {
  if (suppressHashSync) return;
  const targetHash = view === "workbench" ? "" : `#${view}`;
  const current = window.location.hash;
  if (current === targetHash || (view === "workbench" && !current)) return;
...
export function openMobileNavMore() {
  ...
  panel?.querySelector<HTMLElement>(".mobile-nav-overflow-item")?.focus();
}

function syncMobileNavMore(view: string) {
  const moreBtn = document.getElementById("mobile-nav-more-btn");
  moreBtn?.classList.toggle("active", MOBILE_OVERFLOW_VIEWS.has(view));
  document.querySelectorAll<HTMLElement>(".mobile-nav-overflow-item").forEach((item) => {
...
export function switchView(view, { updateHash = true } = {}) {
  if (!VALID_VIEWS.has(view)) view = "workbench";
  tabs.forEach((tab) => {
    tab.classList.toggle("active", tab.dataset.view === view);
  });
  syncMobileNavMore(view);
  if (updateHash) syncViewHash(view);
